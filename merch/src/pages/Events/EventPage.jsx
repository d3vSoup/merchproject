...
export default function EventPage() {
  ...
  return (
    <section className="product-section event-page">
      {/* Universal Hero Status Box — always shown */}
      <div className={`made-for-main-stage made-for-main-stage--${eventKey}${isEventSoldOut ? ' made-for-main-stage--soldout' : ''}`}>
        <div className="made-for-main-stage__content">
          <div className="made-for-main-stage__text">
            <h2 className="made-for-main-stage__headline">
              <RotatingText
                texts={isEventSoldOut ? soldOutHeadlines : showCountdown ? mainStageHeadlines : ongoingHeadlines}
            ...
            <p className="made-for-main-stage__desc">
              {isEventSoldOut
                ? `The ${eventTitle} collection has sold out. Stay tuned for the next drop.`
                : showCountdown
                  ...
                  : `The ${eventTitle} collection is live. Grab your favorites before they\u0027re gone.`
              }
            </p>
            {showCountdown && (
            ...
            <div className="made-for-main-stage__box made-for-main-stage__box--top" style={{ overflow: 'hidden', position: 'relative' }}>
              {heroImages.map((src, idx) => (
                <div
                  ...
                  style={{
                    ...
                    ...(isEventSoldOut ? { filter: 'grayscale(1) brightness(0.7)' } : {}),
                  }}
                />
              ))}
              {heroImages.length === 0 && eventProducts[0]?.imageUrl && (
                <div
                  className="made-for-main-stage__product-img"
                  style={{
                    backgroundImage: `url(${eventProducts[0].imageUrl})`,
                    ...(isEventSoldOut ? { filter: 'grayscale(1) brightness(0.7)' } : {}),
                  }}
                />
              )}
        ...
        <div className={`event-hero-countdown made-for-main-stage__countdown${isEventSoldOut ? ' event-hero-countdown--soldout' : ''}`}>
          {isEventSoldOut && (
            <div className="event-hero-soldout-overlay">
              <span className="event-hero-soldout-text">SOLD OUT</span>
            </div>
          )}
          <FlipClock targetDate={showCountdown ? eventStatus.countdown : null} />
        </div>
      </div>

      {/* Collection title below the hero */}
